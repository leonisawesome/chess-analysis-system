...
class InstructionalLanguageDetector:
    ...
    def _log_compiled_counts(self):
        ...
        try:
            total = 0
            compiled = getattr(self, 'compiled_patterns', {}) or {}
            for _, pats in compiled.items():
                total += len(pats)
            logging.info(f"PID {pid}: compiled_pattern_categories={list(compiled.keys())} total_compiled={total}")
        except Exception as e:
            logging.warning(f"PID {pid}: failed _log_compiled_counts: {e}")

    def _ensure_compiled(self):
        """Ensure compiled_patterns exists and is non-empty in this process."""
        # Some code paths compile into self._compiled; expose a mapping for diagnostics/tests
        if not hasattr(self, 'compiled_patterns') or not getattr(self, 'compiled_patterns'):
            # If _compile_patterns exists, call it to (re)build internals
            if hasattr(self, '_compile_patterns'):
                try:
                    self._compile_patterns()
                except Exception as e:
                    logging.debug(f"_ensure_compiled _compile_patterns failed: {e}")
            # Derive compiled_patterns mapping from _compiled if present
            compiled_map = {}
            try:
                for entry in getattr(self, '_compiled', []) or []:
                    try:
                        cat, regex_obj, orig = entry
                    except Exception:
                        # Fallback for different tuple shapes
                        if isinstance(entry, tuple) and len(entry) >= 2:
                            cat, regex_obj = entry[0], entry[1]
                        else:
                            continue
                    compiled_map.setdefault(canon(cat), []).append(regex_obj)
            except Exception as e:
                logging.debug(f"_ensure_compiled building map failed: {e}")
                compiled_map = {}
            # Also include slot patterns if compiled
            for key in ('compiled_slot_patterns',):
                try:
                    for cat, patterns in (getattr(self, key, {}) or {}).items():
                        compiled_map.setdefault(canon(cat), []).extend(patterns)
                except Exception:
                    pass
            # Attach if anything found
            if compiled_map:
                self.compiled_patterns = compiled_map
        # Log for visibility
        self._log_compiled_counts()

    @classmethod
    def factory(cls):
        """Per-process factory: fresh instance with compiled patterns and diagnostics."""
        inst = cls()
        try:
            inst._ensure_compiled()
        except Exception as e:
            logging.debug(f"factory ensure failed: {e}")
        return inst

    def debug_raw_matches(self, text: str):
        """Return raw matches per category BEFORE any gating for diagnostics."""
        try:
        ...
        for category, patterns in compiled.items():
            ...
            for pat in patterns:
                try:
                    ...
                    logging.debug(f"debug_raw_matches: pattern error in {category}: {e}")
            out[category] = bucket
        return out
...
def detect_instructional(text: str) -> bool:
    ...
    def __init__(self, embedding_model: Optional[SentenceTransformer] = None, 
        ...
        logger.info({
            ...
            "context": "ThreadPoolExecutor_expected"
        })
    ...
    def _load_instructional_vocabulary(self):
        ...
        # PARTNER-VALIDATED DIAGNOSTIC: Pre-load validation
        logger.info("VOCAB DIAGNOSTIC: Starting vocabulary load...")
        logger.info(f"VOCAB DIAGNOSTIC: Import source - INSTRUCTIONAL_LEXICON type: {type(INSTRUCTIONAL_LEXICON)}")
        
        # Load vocabulary components
        self.fixed_phrases = INSTRUCTIONAL_LEXICON
        self.slot_patterns = SLOT_PATTERNS
        self.category_weights = CATEGORY_WEIGHTS  
        self.category_caps = CATEGORY_CAPS
        
        # PARTNER-VALIDATED DIAGNOSTIC: Post-load validation with explicit counts
        if not self.fixed_phrases:
            logger.error("CRITICAL: fixed_phrases is empty - vocabulary loading FAILED")
            raise RuntimeError("Vocabulary loading failure: fixed_phrases empty")
            
        total_phrases = sum(len(phrases) for phrases in self.fixed_phrases.values())
        logger.info(f"VOCAB DIAGNOSTIC: Loaded {len(self.fixed_phrases)} categories, {total_phrases} total phrases")
        
        # PARTNER-VALIDATED DIAGNOSTIC: Per-category validation with expected minimum
        expected_minimum = 50  # Partner consensus: expect 73+ but alert if <50
        if total_phrases < expected_minimum:
            logger.error(f"CRITICAL: Only {total_phrases} patterns loaded, expected >= {expected_minimum}")
            logger.error(f"VOCAB DIAGNOSTIC: Categories loaded: {list(self.fixed_phrases.keys())}")
            raise RuntimeError(f"Vocabulary underflow: {total_phrases} phrases, expected >= {expected_minimum}")
            
        # PARTNER-VALIDATED DIAGNOSTIC: Log detailed per-category counts
        for category, phrases in self.fixed_phrases.items():
            logger.info(f"VOCAB DIAGNOSTIC: {category} = {len(phrases)} phrases: {phrases[:3] if phrases else 'EMPTY'}")
            
        logger.info(f"VOCAB DIAGNOSTIC: Vocabulary loading SUCCESSFUL - {total_phrases} phrases across {len(self.fixed_phrases)} categories")
    ...
    def _compile_patterns(self):
        """Phase 3: Per-pattern compilation with partner-validated diagnostics"""
        
        # PARTNER-VALIDATED DIAGNOSTIC: Pre-compilation validation
        logger.info("COMPILE DIAGNOSTIC: Starting pattern compilation...")
        ...
        if not source:
            logger.error("CRITICAL: Vocabulary not loaded before compilation")
            raise RuntimeError("Lexicon not loaded: fixed_phrases is empty - call _load_instructional_vocabulary() first")
        
        total_input_phrases = sum(len(phrases) for phrases in source.values())
        logger.info(f"COMPILE DIAGNOSTIC: Input - {len(source)} categories, {total_input_phrases} total phrases")
        ...
        # PARTNER-VALIDATED DIAGNOSTIC: Post-compilation validation
        if compiled_count == 0:
            logger.error("CRITICAL: Zero patterns compiled - complete compilation failure")
            raise RuntimeError(f"No patterns compiled from {len(source)} categories - check lexicon data")
        
        # PARTNER-VALIDATED DIAGNOSTIC: Success validation with detailed counts
        logger.info(f"COMPILE DIAGNOSTIC: SUCCESS - {compiled_count} patterns compiled from {total_input_phrases} input phrases")
        
        # PARTNER-VALIDATED DIAGNOSTIC: Per-category compilation results  
        compilation_by_category = {}
        for cat, _, _ in self._compiled:
            compilation_by_category[cat] = compilation_by_category.get(cat, 0) + 1
            
        for category, count in compilation_by_category.items():
            logger.info(f"COMPILE DIAGNOSTIC: {category} = {count} compiled patterns")
            
        # PARTNER-VALIDATED DIAGNOSTIC: Alert if compilation count suggests partial loading
        expected_minimum = 50  # Partner consensus threshold
        if compiled_count < expected_minimum:
            logger.error(f"COMPILE DIAGNOSTIC: UNDERFLOW DETECTED - {compiled_count} compiled vs {expected_minimum} expected minimum")
        else:
            logger.info(f"COMPILE DIAGNOSTIC: Pattern count HEALTHY - {compiled_count} >= {expected_minimum} minimum threshold")
    ...
    def analyze_instructional_language(self, chunks: List[str], debug: bool = False) -> float:
        """
        Phase 4: Sentence-level analysis with partner-validated runtime diagnostics
        Returns normalized instructional language score [0.0, 1.0]
        """
        if not chunks:
            return 0.0
        
        # PARTNER-VALIDATED DIAGNOSTIC: Runtime pattern validation
        if not hasattr(self, '_compiled') or not self._compiled:
            logger.error(f"RUNTIME DIAGNOSTIC: NO compiled patterns at analyze - PID={os.getpid()}, TID={threading.get_ident()}")
            logger.error(f"RUNTIME DIAGNOSTIC: Detector state - ID={id(self)}, fixed_phrases={bool(getattr(self, 'fixed_phrases', None))}")
