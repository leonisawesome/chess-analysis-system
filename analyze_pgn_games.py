...
class PGNAnalyzer:
    ...
    def _create_chunks(self, game: chess.pgn.Game, filename: str, game_num: int) -> List[Dict]:
        ...
        exceeding the 8192 token limit for embeddings.
        """
        ...
        # Testing shows: 1 token ≈ 3 characters for annotated PGN (not 4)
    ...
    def _split_large_game(self, game: chess.pgn.Game, filename: str, game_num: int, metadata: Dict) -> List[Dict]:
        ...
        Strategy:
        - Count total moves in mainline
        - Split into chunks of ~20-30 moves each
        - Each chunk includes context (breadcrumb + summary + moves)
        - Preserve all variations and annotations for the moves in that range
        """
        ...
        if move_count == 0:
            ...
            return [{
                "chunk_id": f"{Path(filename).stem}_game_{game_num}_part1",
                "text": chunk_text,
                "metadata": {**metadata, "part_num": 1, "total_parts": 1},
                "token_estimate": token_estimate
            }]
        ...
        # For now, use simple text-based splitting
        # (More sophisticated move-based splitting can be added later)
        chars_per_part = len(game_str) // total_parts

        for part_num in range(1, total_parts + 1):
            # Calculate character range for this part
            start_char = (part_num - 1) * chars_per_part
            end_char = start_char + chars_per_part if part_num < total_parts else len(game_str)

            # Add overlap for context (10% of previous chunk)
            overlap = chars_per_part // 10
            if part_num > 1:
                start_char = max(0, start_char - overlap)

            # Extract this part of the game
            game_part = game_str[start_char:end_char]

            # Calculate move range (approximate)
            start_move = ((part_num - 1) * MOVES_PER_CHUNK) + 1
            end_move = min(part_num * MOVES_PER_CHUNK, move_count)

            # Create chunk text with part information
            chunk_text = self._create_chunk_text(
                game, metadata,
                part_num=part_num,
                total_parts=total_parts,
                move_range=(start_move, end_move),
                game_part_text=game_part
            )
            ...
            chunk_metadata = {
                **metadata,
                "part_num": part_num,
                "total_parts": total_parts,
                "move_range": f"{start_move}-{end_move}"
            }

            chunks.append({
                "chunk_id": f"{Path(filename).stem}_game_{game_num}_part{part_num}",
                "text": chunk_text,
    ...
    def _create_chunk_text(self, game: chess.pgn.Game, metadata: Dict,
        ...
        Args:
            ...
            part_num: Part number if this is a split chunk
            total_parts: Total parts if this is a split chunk
            move_range: (start_move, end_move) tuple if split
            game_part_text: The PGN text for this part (if split)
        """
        lines = []

        # Breadcrumb header (for embedding)
        breadcrumb = self._create_breadcrumb(metadata, part_num, total_parts, move_range)
        lines.append(breadcrumb)
        lines.append("")

        # Game summary
        summary = self._create_summary(metadata, game, part_num, total_parts, move_range)
        lines.append(summary)
        lines.append("")

        # PGN (either full game or part)
        lines.append("PGN:")
        if game_part_text:
    ...
    def _create_breadcrumb(self, metadata: Dict, part_num: int = None,
                           total_parts: int = None, move_range: tuple = None) -> str:
        ...
        # Part information (for split games)
        if part_num and total_parts:
            if move_range:
                parts.append(f"Part {part_num}/{total_parts} (Moves {move_range[0]}-{move_range[1]})")
            else:
                parts.append(f"Part {part_num}/{total_parts}")

        return " ▸ ".join(parts)

    def _create_summary(self, metadata: Dict, game: chess.pgn.Game,
                       part_num: int = None, total_parts: int = None,
                       move_range: tuple = None) -> str:
        """Create summary line with key game info."""
        parts = []

        # Part information first (if split)
        if part_num and total_parts and move_range:
            parts.append(f"Part {part_num}/{total_parts}: Moves {move_range[0]}-{move_range[1]}")
    ...
    def print_stats(self):
        ...
        print(f"Games split: {self.stats.get('games_split', 0)} (oversized)")
