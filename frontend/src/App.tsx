...
import { BookOpen, Search, Layout, Settings, ChevronRight, Loader, ChevronLeft, FastForward, Rewind, Play, Image as ImageIcon, Copy } from 'lucide-react';
...
function App() {
  ...
  const handleMoveClick = (clickedMove: string, fullText: string) => {
    ...
    // Attempt 2: Sequence from diagram
    try {
      const moveRegex = /((?:\d{1,3}(?:\.\.\.|\.)\s*)?[KQRBN]?[a-h]?[1-8]?x?[a-h][1-8](?:\=[QRBN])?[\+#]?|O-O(?:-O)?)/g;
    ...
    <div className={`flex h-screen bg-board-dark text-paper font-sans overflow-hidden ${isResizingSidebar || isResizingBoard ? 'select-none cursor-col-resize' : ''}`}>
      ...
      <aside style={{ width: boardWidth }} className="bg-board-dark/95 border-l border-white/10 p-8 flex flex-col shadow-2xl shrink-0">
        <div className="aspect-square w-full shadow-[0_0_50px_rgba(0,0,0,0.5)] rounded-lg overflow-hidden border-[12px] border-board-panel">
          {/* Robustness Key: Force re-render on activeFen change if needed */}
          <Chessboard
            key={activeFen}
            options={{
              position: activeFen,
        ...
        <div className="mt-8 flex flex-col flex-1 min-h-0 bg-black/30 rounded-xl p-6 border border-white/5 shadow-inner">
          ...
          <div className="flex-1 overflow-y-auto mb-6">
            {currentVariation ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center px-4"><Play size={32} className="text-white/10 mb-2" /><p className="text-paper/30 text-sm font-serif italic">Select a move to start line analysis.</p></div>
            )}
          </div>
          <div className="flex items-center justify-between text-[10px] font-mono text-paper/40 mb-2 truncate">
          ...
          <button className="w-full bg-board-accent text-board-dark font-black tracking-tighter text-sm h-14 rounded-lg hover:brightness-110 active:scale-[0.98] transition-all uppercase shrink-0">Deep Analysis</button>
        </div>
      </aside>
    </div>
  );
