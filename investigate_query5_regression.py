#!/usr/bin/env python3
"""
INVESTIGATION: Query 5 Regression (90% → 60%)
...
QUERY_5 = "What is the best way to study chess openings?"
...
def investigate_query5():
    """Compare baseline vs expanded for Query 5"""

    print("="*80)
    print("QUERY 5 REGRESSION INVESTIGATION")
    print("="*80)
    print(f"\nOriginal Query: {QUERY_5}")
    print()

    # Test 1: Baseline (no expansion, 20 candidates)
    print("="*80)
    print("BASELINE: No expansion, Top 20 candidates (GPT-5 baseline)")
    print("="*80)

    candidates_baseline = get_semantic_candidates(QUERY_5, limit=20)
    ranked_baseline = gpt5_rerank_baseline(QUERY_5, candidates_baseline)
    top5_baseline = ranked_baseline[:5]

    print("\nTop 5 results (BASELINE - achieved 90% precision):")
    ...
    # Test 2: Expanded query
    print("\n" + "="*80)
    print("EXPANDED QUERY TEST")
    print("="*80)

    expanded = expand_query(QUERY_5)
    print(f"\nExpanded Query Text:")
    print(f"'{expanded}'")
    print()

    candidates_expanded = get_semantic_candidates(expanded, limit=40)
    ranked_expanded = gpt5_rerank_baseline(QUERY_5, candidates_expanded)  # Note: rerank with ORIGINAL query
    top5_expanded = ranked_expanded[:5]

    print("\nTop 5 results (EXPANDED - achieved 60% precision):")
    for i, (cand, score) in enumerate(top5_expanded, 1):
        book = cand.payload.get('book_name', 'unknown')
        chapter = cand.payload.get('chapter_title', 'unknown')
        text = cand.payload['text'][:150].replace('\n', ' ')

        # Check if this result was in baseline top-5
    ...
    # Count how many baseline top-5 were lost
    ...
    baseline_avg_sem = sum(c.score for c, _ in top5_baseline) / 5
    expanded_avg_sem = sum(c.score for c, _ in top5_expanded) / 5
    print(f"  Baseline avg semantic score: {baseline_avg_sem:.4f}")
    print(f"  Expanded avg semantic score: {expanded_avg_sem:.4f}")
    print(f"  Change: {expanded_avg_sem - baseline_avg_sem:+.4f}")

    # Check if expansion made query too specific
    print(f"\nExpansion length analysis:")
    print(f"  Original query length: {len(QUERY_5)} chars")
    print(f"  Expanded query length: {len(expanded)} chars")
    print(f"  Expansion ratio: {len(expanded)/len(QUERY_5):.1f}x")

    if len(expanded) / len(QUERY_5) > 10:
